...
//NOTES(): EWWW
#include <stdarg.h>
#define S32(newstring)  vCreateString(newstring,__FILE__,__LINE__);
#define S32M(newstring,file,line)  vCreateString(newstring,file,line);
//#define S32Cat(source,...) _S32Cat(__FILE__,__LINE__,source,...);
#define MidString(source,from,to) vMidString(source,from,to,__FILE__,__LINE__) 
#define FindString(stringToFind, sourceString)  FindString_(stringToFind, sourceString, __FILE__,__LINE__)
//#define FindStringA(stringToFind, sourceString)  FindString__(stringToFind, sourceString, __FILE__,__LINE__)
#define ToLower(string) ToLower_(string,__FILE__,__LINE__)
#define  StrSplit(source,determinator) StrSplit_(source, determinator,__FILE__, __LINE__)
typedef char  s32;
...
s32* S32Cat( s32* source,i32 size,...)
{
	...
	for (i=0;i<size;i++)
	{
		...
		temp = MemoryA(char,Strlen(result) + 1 + Strlen(stringArg) + 4);
